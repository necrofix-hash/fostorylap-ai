      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 pt-16 pb-20 sm:px-6 lg:px-8 flex flex-col items-center text-center">
        ...
        <div className="w-full max-w-2xl relative group mb-12">
            ...
            <div className="relative flex items-stretch">
                ...
                <button className="ml-2 bg-black text-white px-8 font-bold text-lg border-2 border-transparent hover:bg-neutral-800 shadow-[4px_4px_0px_0px_#888] active:translate-y-1 active:shadow-none transition-all hidden sm:block">
                    SEARCH
                </button>
            </div>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3">
            {HERO_TAGS.map((tag) => (
                <span key={tag} className="px-3 py-1 bg-white border-2 border-black font-mono text-sm font-bold hover:bg-black hover:text-white cursor-pointer transition-colors shadow-[2px_2px_0px_0px_#000]">
                    {tag}
                </span>
            ))}
        </div>
